...
Special providers automate the build process with configurable virtual containers. This is especially useful for teams in need of a shared build environment. The provider gets access to the repository (usually Git based), automatically pulls new content and (re)builds the site or a preview. Some of them also own a CDN, which gets populated immediately afterwards.
...
Person(editor, "Editor", "Edits content")
Person(author, "Author", "Writes Content" )
...
Rel_R(editor, local2, "Checks", "draft/content preview")
Rel(editor, build, "Assures", "content preview")
...
in a build container is essential if you don’t want to waste energy and money for senseless build-time. Many different versions of every image are generated by this theme. These files are always cached by Hugo in the reserved {$resources} folder and every image needs to be processed only once. But you need to make sure that this caching folder is stored in your repository or enable another caching mechanism between subsequent Hugo container runs, because your container is initialized from scratch for every build.
